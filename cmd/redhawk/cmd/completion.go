...
const (
	...
	OS X:
		...
		$ redhawk completion bash > ~/.redhawk-completion  # for bash users
		$ redhawk completion zsh > ~/.redhawk-completion   # for zsh users
		$ source ~/.redhawk-completion
	Ubuntu:
		$ apt-get install bash-completion
		$ source /etc/bash-completion
		$ source <(redhawk completion bash) # for bash users
		$ source <(redhawk completion zsh)  # for zsh users

	Additionally, you may want to output the completion to a file and source in your .bashrc
`

	zshCompdef = "\ncompdef _redhawk redhawk\n"
)
